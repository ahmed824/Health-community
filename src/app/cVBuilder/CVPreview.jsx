...
const CVPreview = ({
  ...
  <div className="cv-preview bg-white p-4 rounded-lg min-h-[800px] print:border-0 print:shadow-none"
    style={{
  ...
  >
    {/* Header */}
    <header
      className="flex flex-col sm:flex-row items-start sm:items-center mb-4 pb-2 border-b-2 gap-4"
    ...
    >
      ...
      {cvData.personalInfo.photoPreview && (
        <img
          ...
          alt="Profile"
          className="object-cover w-24 h-24 rounded-full border-2 border-gray-300 shadow mr-4 mb-2 sm:mb-0"
          style={{ background: "#fff" }}
        />
      )}
      <div className="flex-1">
        <h1 className="text-3xl font-bold mb-1" style={{ color: "#000" }}>
          {cvData.personalInfo.fullName || "Your Name"}
        </h1>
        {cvData.personalInfo.jobTitle && (
          <div className="text-lg font-semibold text-gray-700 mb-1" style={{ color: "#323332" }}>
            {cvData.personalInfo.jobTitle}
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2 text-xs mb-1" style={{ color: "#323332" }}>
          {cvData.personalInfo.email && <span>{cvData.personalInfo.email}</span>}
          {cvData.personalInfo.email && cvData.personalInfo.phone && <span>|</span>}
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs mb-1" style={{ color: "#323332" }}>
          ...
          {cvData.personalInfo.country && (
            <span>
              {cvData.personalInfo.country.charAt(0).toUpperCase() + cvData.personalInfo.country.slice(1)}
            </span>
          )}
          {cvData.personalInfo.city && (
            <span>
              {cvData.personalInfo.city.charAt(0).toUpperCase() + cvData.personalInfo.city.slice(1)}
            </span>
          )}
          {cvData.personalInfo.nationality && (
            <span>Nationality: {cvData.personalInfo.nationality}</span>
          )}
          {cvData.personalInfo.dob && <span>DOB: {cvData.personalInfo.dob}</span>}
        </div>
        {/* Social Links */}
        {cvData.personalInfo.socialLinks && cvData.personalInfo.socialLinks.length > 0 && (
          <div className="flex flex-wrap items-center gap-3 mt-1">
            {cvData.personalInfo.socialLinks.filter((link) => link.type && link.url).map((link, idx) => {
              let icon = null;
              switch (link.type) {
                case "linkedin":
                  icon = (
                ...
                case "facebook":
                  icon = (
                    <svg width="18" height="18" fill="#1877F3" viewBox="0 0 24 24">
                ...
                case "instagram":
                  icon = (
                    <svg width="18" height="18" fill="#E4405F" viewBox="0 0 24 24">
                ...
                case "website":
                  icon = (
                    <svg width="18" height="18" fill="#617A78" viewBox="0 0 24 24">
    ...
    <section className="mb-3">
      <h2 className="text-lg font-bold mb-1 border-b pb-1" style={{ color: "#000", borderColor: "#323332" }}>
      ...
      {cvData.experience && cvData.experience.length > 0 ? (
        <ul>
          {cvData.experience.map((exp, index) => (
            <li
              key={exp.id}
              className={`mb-2 ${index < cvData.experience.length - 1 ? "pb-2 border-b" : ""}`}
              style={index < cvData.experience.length - 1 ? { borderColor: "#323332" } : {}}
            >
              <div className="flex justify-between items-start mb-1">
                <div>
                  ...
                  <p className="text-sm font-semibold" style={{ color: "#323332" }}>
                    {exp.company || "Company Name"}
                    {(exp.country || exp.city) && (
                      <span className="ml-2 text-xs font-normal" style={{ color: "#323332" }}>
                        {exp.country && exp.country.charAt(0).toUpperCase() + exp.country.slice(1)}
                        {exp.country && exp.city ? ", " : ""}
                        {exp.city && exp.city.charAt(0).toUpperCase() + exp.city.slice(1)}
                      </span>
                    )}
                  </p>
                </div>
                <span className="text-xs font-medium bg-white px-2 py-0.5 rounded-full" style={{ color: "#323332", border: "1px solid #323332" }}>
                  {formatExperienceDuration(exp) || "Duration"}
                </span>
              </div>
              {exp.description && (
                <p className="leading-tight text-justify text-xs" style={{ color: "#323332" }}>
                  {exp.description}
                </p>
              )}
            </li>
        ...
        <div className="text-xs text-gray-500 italic">No work experience added yet.</div>
      )}
    </section>
      ...
      {cvData.education && cvData.education.length > 0 ? (
        <ul>
          {cvData.education.map((edu, index) => (
            <li
              key={edu.id}
              className={`mb-2 ${index < cvData.education.length - 1 ? "pb-2 border-b" : ""}`}
              style={index < cvData.education.length - 1 ? { borderColor: "#323332" } : {}}
            >
              <div className="flex justify-between items-start mb-1">
                <div>
                  <h3 className="text-base font-bold" style={{ color: "#000" }}>
                  ...
                  <p className="text-sm font-semibold" style={{ color: "#323332" }}>
                    ...
                    {(edu.country || edu.city) && (
                      <span className="ml-2 text-xs font-normal" style={{ color: "#323332" }}>
                        {edu.country && edu.country.charAt(0).toUpperCase() + edu.country.slice(1)}
                        {edu.country && edu.city ? ", " : ""}
                        {edu.city && edu.city.charAt(0).toUpperCase() + edu.city.slice(1)}
                      </span>
                    )}
                  </p>
                </div>
        ...
        <div className="text-xs text-gray-500 italic">No education added yet.</div>
      )}
    </section>
    ...
    <section className="mb-3">
      ...
      {cvData.skills && cvData.skills.length > 0 ? (
        <ul className="grid grid-cols-1 gap-1">
          {cvData.skills.map((skill) => (
            <li key={skill.id} className="flex items-center gap-2">
              <span className="font-semibold min-w-[100px] text-xs" style={{ color: "#000" }}>
              ...
              <span className="text-xs font-medium min-w-[60px] text-right" style={{ color: "#323332" }}>
                {skill.level}
              </span>
            </li>
          ))}
        ...
        <div className="text-xs text-gray-500 italic">No skills added yet.</div>
      )}
    </section>
      ...
      {cvData.achievements && cvData.achievements.length > 0 ? (
        <ul>
          {cvData.achievements.map((achievement, index) => (
            <li
              key={achievement.id}
              className={`mb-2 ${index < cvData.achievements.length - 1 ? "pb-2 border-b" : ""}`}
              style={index < cvData.achievements.length - 1 ? { borderColor: "#323332" } : {}}
            >
        ...
        <div className="text-xs text-gray-500 italic">No achievements or awards added yet.</div>
      )}
    </section>
    ...
    <section className="mb-3">
      ...
      {cvData.languages && cvData.languages.length > 0 ? (
        ...
        <div className="text-xs text-gray-500 italic">No languages added yet.</div>
      )}
    </section>
    ...
    <section className="mb-3">
      ...
      {cvData.certificates && cvData.certificates.length > 0 ? (
        <ul className="grid grid-cols-1 gap-1">
          {cvData.certificates.map((cert, idx) => (
            <li key={cert.id} className="flex items-center gap-2">
              ...
              {/* File type */}
              {cert.type === "file" && cert.filePreview && (
        ...
        <div className="text-xs text-gray-500 italic">No certificates added yet.</div>
      )}
    </section>
    ...
    {!isFormValid && (
      <section className="text-center py-10">
        <div className="mb-2">
          <h3 className="text-base font-medium mb-1" style={{ color: "#323332" }}>
